...
void Beep(uint16_t Period, uint8_t Duty) // beep frequency = CPUclk/2/Period = 30000000/Period
{ 
  uint32_t Limit = ((uint32_t)Period * (uint32_t)Duty + 0x0080) >> 8;
  if(Limit) TIM_OC4PolarityConfig(TIM4, TIM_OCPolarity_High);
       else TIM_OC4PolarityConfig(TIM4, TIM_OCPolarity_Low);
  TIM_SetCompare3(TIM4, (uint16_t)Limit); // TIM4->CCR3 = Limit;
  TIM_SetCompare4(TIM4, (uint16_t)Limit); // TIM4->CCR4 = Limit;
